...
class Invoice(models.Model):
    ...
    invoice_number = models.CharField(max_length=100, unique=True, db_index=True, editable=False)
    source_quotation = models.ForeignKey(
        ...
        help_text='Quotation this invoice was converted from'
    )
    client = models.ForeignKey(Client, on_delete=models.PROTECT, related_name='invoices')
    template = models.ForeignKey(DocumentTemplate, on_delete=models.SET_NULL, null=True, blank=True, related_name='invoices')
    ...
    def __str__(self):
        return f"{self.invoice_number} - {self.client.client_name} ({self.get_status_display()})"

    def calculate_totals(self):
        """
        Backend source of truth for invoice calculations.
        """
